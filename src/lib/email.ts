...
export interface EmailDispatchResult {
  ...
  resendId?: string;
  error?: string;
  status: 'sent' | 'rate_limited' | 'failed' | 'bypassed';
  targetEmail?: string;
}
...
async function logNotificationToDb({
  ...
  senderName?: string;
  subject?: string;
  status: 'sent' | 'failed' | 'rate_limited' | 'bypassed';
  resendId?: string;
  errorMessage?: string;
  ...
  try {
    ...
    // 1. Log to email_notification_logs table
    ...
    // 2. Also register in public.notifications for in-app alert sync
...
// ==============================================================================
// RESILIENT SENDER WITH EXPONENTIAL RETRY
// ==============================================================================
