  ...
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 1000, background: isScrolled ? 'rgba(255, 255, 255, 0.85)' : '#ffffff', backdropFilter: isScrolled ? 'blur(12px)' : 'none', borderBottom: '1px solid var(--border-color)', transition: 'all 0.3s ease', boxShadow: isScrolled ? 'var(--shadow-sm)' : 'none' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', width: '100%', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {/* Center Search Bar (Fiverr Style - Desktop) */}
        {safeCurrentView === 'public' && (
          <div className="desktop-only" style={{ flex: 1, maxWidth: '500px', margin: '0 2rem' }}>
            <div className="marketplace-search">
              ...
              <button 
                type="button" 
                className="marketplace-search-btn"
      ...
      {/* Secondary Marketplace Category SubNav (Desktop) */}
      {safeCurrentView === 'public' && (
        <div className="marketplace-subnav desktop-only">
          {[
            { id: 'all', label: 'All Services', path: '/' },
            { id: 'embroidery', label: 'Embroidery Digitizing', path: '/services/embroidery-digitizing' },
            { id: 'vector-art', label: 'Vector Art', path: '/services/vector-tracing' },
            { id: 'patches', label: 'Custom Patches', path: '/custom-patches' },
            { id: 'portfolio', label: 'Portfolio', path: '/portfolio' },
            { id: 'pricing', label: 'Pricing', path: '/pricing' },
            { id: 'faqs', label: 'FAQs', path: '/faqs' }
          ].map(tab => {
            const isActive = currentPath === tab.path || (tab.id === 'all' && currentPath === '/') || (activeHomeServiceTab === tab.id && currentPath === '/');
            return (
              <button
                key={tab.id}
                className={`marketplace-subnav-link ${isActive ? 'active' : ''}`}
                onClick={() => {
                  setCurrentView('public');
                  if (tab.id === 'all') {
                    if (setActiveHomeServiceTab) setActiveHomeServiceTab('all');
                    navigate('/');
                  } else {
                    navigate(tab.path);
                  }
