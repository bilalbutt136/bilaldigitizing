  ...
  return (
    <div style={{ padding: '1rem 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--navy-900)' }}>Dynamic Pricing Tiers</h2>
          <p style={{ color: 'var(--text-muted)' }}>Manage storefront pricing, features, and display details.</p>
        </div>
            ...
            filteredTiers.map(tier => (
              <div key={tier.id} className="card" style={{ padding: '1.5rem', background: '#fff', borderRadius: '12px', border: tier.is_popular ? '2px solid var(--orange-500)' : '1px solid var(--border-color)', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
                  <button type="button" onClick={() => handleEdit(tier)} className="btn btn-outline btn-sm" style={{ flex: 1 }}>
                    <Edit2 size={14} /> Edit
                  </button>
                  <button type="button" onClick={() => handleDelete(tier.id)} className="btn btn-outline btn-sm" style={{ padding: '0.5rem', color: 'red', borderColor: 'red' }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
