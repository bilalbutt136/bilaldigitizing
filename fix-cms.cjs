...
const startIdx = code.indexOf('const handleSaveAll = async () => {');
const returnIdx = code.indexOf('return (', startIdx);
// Find the previous newline or space
let endIdx = returnIdx;
while (endIdx > 0 && code[endIdx-1] !== '\\n' && code[endIdx-1] !== '\\r') {
  endIdx--;
}
...
if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
  ...
  console.log('Fixed UnifiedCmsManager.jsx save handlers');
} else {
