...
async function generateHelpDeskAutoReply({ clientName, targetEmail, latestText, attachmentUrl, history }) {
  const apiKey = (
    ...
    if (action === 'fetchConversations') {
      ...
      rawMessages.forEach(m => {
        ...
        const mappedMsg = {
          ...
          attachment_name: attachName || (resolvedOfferData ? `Custom Offer: ${resolvedOfferData.title}` : (typeof m.attachment === 'string' && !m.attachment.trim().startsWith('{') && m.attachment.trim().length > 0 ? m.attachment : null)),
  ...
  try {
    ...
    if (action === 'upsertConversation') {
      ...
      const dbPayload = {
        ...
        channel: isSupport ? 'support' : 'inbox',
    ...
    if (action === 'insertMessage') {
      ...
      // 3. Autonomous 24/7 Live Support Auto-Pilot Engine
      ...
      if (isSupport && actualSender === 'client' && !payload.is_autopilot && !payload.auto_pilot && payload.text && String(payload.text).trim()) {
        try {
          // Check if 24/7 helpdesk auto-pilot is enabled (default true)
          ...
          if (isAutoPilotOn) {
            // Fetch last 6 messages from this conversation for context
            let recentHistory = [];
            try {
              const { data: hist } = await supabase
                .from('messages')
                .select('sender, text, sender_name, attachment_name, attachment_url')
                .eq('conversation_id', canonicalConvId)
                .order('created_at', { ascending: false })
                .limit(6);
              if (Array.isArray(hist)) {
                recentHistory = hist.reverse();
              }
            } catch {}

            const replyText = await generateHelpDeskAutoReply({
              clientName: actualSenderName || 'Client',
            ...
            if (replyText && replyText.trim()) {
              ...
              // Insert auto-reply to database
              const { data: autoInsData } = await supabase.from('messages').insert([autoMsgPayload]).select();
              if (autoInsData && autoInsData[0]) {
                autoReplyMsg = autoInsData[0];
              } else {
                autoReplyMsg = autoMsgPayload;
              }

              // Update conversation client unread and activity
              try {
                await supabase.from('conversations')
                  .update({
                    updated_at: autoNowIso,
          ...
          console.warn('[Auto-Pilot Error in messages route]:', apErr);
        }
      }
