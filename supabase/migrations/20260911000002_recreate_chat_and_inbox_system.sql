-- ==============================================================================
-- MIGRATION: 20260911000002_recreate_chat_and_inbox_system.sql
-- Description:
-- 1. Recreates public.conversations table with real-time indicators, unread counters, and star flags
-- 2. Recreates public.messages table with support for embroidery/vector attachments, custom offers, and read receipts
-- 3. Creates public.saved_replies table for quick studio auto-replies
-- 4. Seeds default studio canned response templates
-- 5. Enables Supabase Realtime publication on all chat tables
-- 6. Configures Row Level Security (RLS) policies
-- 7. Ensures chat-attachments storage bucket exists with 50MB limits
-- ==============================================================================
...
-- Ensure all columns exist in case table existed
...
CREATE TABLE IF NOT EXISTS public.messages (
    ...
    client_email text NOT NULL,
    sender text NOT NULL DEFAULT 'client', -- 'admin' or 'client'
    sender_name text,
    sender_email text,
    text text DEFAULT '',
    type text DEFAULT 'text', -- 'text', 'attachment', 'custom_offer'
...
-- 3. SAVED REPLIES (AUTO REPLIES) TABLE
...
-- Allow service role full access and authenticated users / clients to access their conversations
...
CREATE POLICY "Saved replies access policy" ON public.saved_replies
    FOR ALL
    USING (true)
    WITH CHECK (true);

-- 6. ENSURE CHAT-ATTACHMENTS STORAGE BUCKET EXISTS
DO $$
BEGIN
    ...
    VALUES (
        ...
        true,
        52428800, -- 50MB
        NULL      -- Unrestricted: allows all machine and artwork formats
    )
...
-- Enable storage RLS policy for chat-attachments
DROP POLICY IF EXISTS "Chat attachments public read policy" ON storage.objects;
CREATE POLICY "Chat attachments public read policy" ON storage.objects
    FOR SELECT
    USING (bucket_id = 'chat-attachments');

DROP POLICY IF EXISTS "Chat attachments upload policy" ON storage.objects;
CREATE POLICY "Chat attachments upload policy" ON storage.objects
    FOR INSERT
