-- ==============================================================================
-- MIGRATION: 20260907000004_automated_email_notifications.sql
...
BEGIN
    ...
    -- Avoid notifying if sender email equals recipient email
    IF LOWER(v_recipient_email) = LOWER(COALESCE(NEW.client_email, '')) AND LOWER(COALESCE(NEW.sender, '')) = 'client' THEN
        -- This is normal admin route
    END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Ensures the message insert NEVER fails even if network or pg_net has an issue
